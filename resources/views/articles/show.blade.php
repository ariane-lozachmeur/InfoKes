...
<div class="row white bloc-padding">
	<h5>Commentaires</h5>
	@unless(isset($session['user']))
	<p> <a class="modal-trigger" href="#modalLogin">Connecte-toi  via Frankiz</a> pour avoir accès aux commentaires (réservé aux élèves).</p>
	@else
	...
	<a class="anchor" id="write"></a>
		...
		<div class="row">
			<div class="input-field col s12">
				<input name="auteur" id="auteur" type="text" class="validate" value="{{$session['user']['name']}}" readonly required>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $(document).on('submit', '#comment', function(e) {  
        ...
        var user = {!!json_encode($session['user'])!!};

        $.ajax({
    		headers: {'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')},
            method: 'POST',
            url: article.id+"/commentaire",
            data: {auteur : user.name, contenu : contenu, id : user.id},
            dataType:"json", 
        })
        .done(function(data) {
            if(data.message=="success"){
        		...
        		//$('#auteur').val('');
        		$('#contenu').val('')
        		Materialize.toast('Ton commentaire a bien été envoyé',2000,'message_success')
                }
            else {
            	Materialize.toast('Il y a un problème avec les commentaires.',2000,'message_error');
            	alert(data.message)
            }	
        })
        .fail(function(data) {
